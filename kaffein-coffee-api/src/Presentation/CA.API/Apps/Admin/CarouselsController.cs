...
namespace CA.API.Apps.Admin
{
    ...
    {
        readonly ICarouselService _service;

        public CarouselsController(ICarouselService service)
        {
            _service = service;
        }
        ...
        [Authorize(Roles = "SuperAdmin")]
        [HttpGet("/api/v1/admin/Carousels/GetWithAllLanguages/{id}")]
        [ProducesResponseType(StatusCodes.Status200OK, Type = typeof(CarouselGetDto))]
        [ProducesResponseType(StatusCodes.Status404NotFound)]
        ...
        public async Task<IActionResult> Create([FromForm] CarouselPostDto carouselPostDto)
        {
            await _service.CreateAsync(carouselPostDto);
        ...
        [ProducesResponseType(StatusCodes.Status404NotFound)]
        public async Task<IActionResult> Update([FromForm] CarouselUpdateDto carouselUpdateDto, int id)
        {
            await _service.UpdateAsync(carouselUpdateDto, id);
